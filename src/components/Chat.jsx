    ...
    <div className="flex items-center justify-center h-full">
      <div className="w-1/3 h-[600px] bg-white relative">
          ...
          {messageList &&
            messageList.map((msg, i) => (
              ...
              >
                <div
                  className={`${
                    username === msg.username ? "bg-green-600 rounded-br-none" : "bg-blue-600 rounded-bl-none"
                  } w-2/3 h-12 text-white text-sm m-2 p-2 rounded-xl`}
                >
                  <div>{msg.message}</div>
        ...
        <div className="absolute bottom-0 left-0 w-full">
          ...
          <button
            onClick={sendMessage}
            className="w-1/4 bg-indigo-600 h-12 text-white hover:opacity-70"
          >
            SEND
          </button>
        </div>
